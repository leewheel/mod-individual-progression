...
void IndividualProgression::checkHunterPetSpells(Player* player)
{
    ...
    auto reconcile = [pet, player](std::initializer_list<uint32> wipeIds, std::initializer_list<uint32> dummyIds)
        {
            ...
            for (uint32 id : wipeIds)
            {
                if (id != keepId && pet->HasSpell(id))
    ...
    if (familyName == "Wolf")
    {
        ...
        reconcile({ 24604, 64491, 64492, 64493, 64494 }, { 624604, 664491, 664492, 664493 });
    ...
    else if (familyName == "Boar")
    {
        ...
        // Charge
        reconcile({ 61685 }, { 661685 }); // 607371, 626177, 626178, 626201, 627685
        ...
        if (sIndividualProgression->hasPassedProgression(player, PROGRESSION_PRE_TBC)) // can learn if player has reached TBC
            reconcile({ 35290, 35291, 35292, 35293, 35294, 35295 }, { 635290, 635291, 635292, 635293, 635294 });
        else
    ...
    else if (familyName == "Wind Serpent")
    {
        ...
        // Lightning Breath
        reconcile({ 24844, 25008, 25009, 25010, 25011, 25012 }, { 624844, 625008, 625009, 625010, 625011, 625012 }); // spell levels don't match, damage is fine, need to check required minimum levels
    ...
    else if (familyName == "Dragonhawk")
    {
        ...
        reconcile({ 34889, 35323, 55482, 55483, 55484, 55485 }, { 634889, 635323 });
    ...
    else if (familyName == "Sporebat")
    {
        ...
        reconcile({ 50274, 53593, 53594, 53596, 53597, 53598 }, {});
    }
    ...
    {
        reconcile({ 17253, 17255, 17256, 17257, 17258, 17259, 17260, 17261, 27050 }, {}); // Bite, ranks 1-9
        reconcile({ 35387, 35389, 35392, 55555, 55556, 55557 }, {}); // Poison Spit
        reconcile({ 61684 }, {}); // Dash
        reconcile({ 1742 }, {}); // Cower
    }
    else if (familyName == "Moth") // WotLK
    {
        reconcile({ 50318, 52012, 52013, 52014, 52015, 52016 }, {});  // Serenity Dust
        reconcile({ 49966, 49967, 49968, 49969, 49970, 49971, 49972, 49973, 49974, 49975, 49976 }, {}); // Smack
        reconcile({ 52825 }, {}); // Swoop
        reconcile({ 23145 }, {}); // Dive
        reconcile({ 1742 }, {}); // Cower
    }
    else if (familyName == "Chimaera") // WotLK
    {
        reconcile({ 17253, 17255, 17256, 17257, 17258, 17259, 17260, 17261, 27050 }, {}); // Bite, ranks 1-9
        reconcile({ 54644, 55488, 55489, 55490, 55491, 55492 }, {}); // Froststorm Breath
        reconcile({ 23145 }, {});  // Dive
        reconcile({ 1742 }, {}); // Cower
    }
    else if (familyName == "Silithid") // WotLK
    {
        reconcile({ 16827, 16828, 16829, 16830, 16831, 16832, 3010, 3009, 27049 }, {}); // Claw, ranks 1-9
        reconcile({ 54706, 55505, 55506, 55507, 55508, 55509 }, {}); // Venom Web Spray
        reconcile({ 61684 }, {}); // Dash
        reconcile({ 1742 }, {}); // Cower
    }
}
...
void IndividualProgression::checkWarlockPetSpells(Player* player)
{
    ...
    case NPC_IMP:
        ...
        reconcile({ 7799, 7800, 7801, 7802, 11762, 11763, 27267, 47964 }, { 603110, 607799, 607800, 607801, 607802, 611762, 611763, 627267 }); // ranks 1-8

        // Blood Pact: ranks 1..7 (47982 is WotLK rank 7)
        reconcile({ 6307, 7804, 7805, 11766, 11767, 27268, 47982 }, { 606307, 607804, 607805, 611766, 611767, 627268 });

        // Fire Shield: ranks 1..7 (47983 is WotLK rank 7)
        reconcile({ 2947, 8316, 8317, 11770, 11771, 27269, 47983 }, { 602947, 608316, 608317, 611770, 611771, 627269 });

        // Phase Shift (single rank)
        reconcile({ 4511 }, { 604511 });
        break;

    case NPC_VOIDWALKER:
        // Torment: ranks 2..7 (47985 is WotLK rank 8)
        reconcile({ 7809, 7810, 7811, 11774, 11775, 27270, 47984 }, { 603716, 607809, 607810, 607811, 611774, 611775, 627270 }); // ranks 1-7

        // Sacrifice: ranks 1..9 (47986 is WotLK rank 9)
        reconcile({ 7812, 19438, 19440, 19441, 19442, 19443, 27273, 47985, 47986 }, { 607812, 619438, 619440, 619441, 619442, 619443, 627273 });

        // Consume Shadows: ranks 1..9 (47988 is WotLK rank 9)
        reconcile({ 17767, 17850, 17851, 17852, 17853, 17854, 27272, 47987, 47988 }, { 617767, 617850, 617851, 617852, 617853, 617854, 627272 });

        // Suffering: ranks 1..8 (47990 is WotLK rank 8)
        reconcile({ 17735, 17750, 17751, 17752, 27271, 33701, 47989, 47990 }, { 617735, 617750, 617751, 617752, 627271, 633701 });
        break;

    case NPC_SUCCUBUS:
        // Lash of Pain: ranks 1..9 (47992 is WotLK rank 9)
        reconcile({ 7814, 7815, 7816, 11778, 11779, 11780, 27274, 47991, 47992 }, { 607814, 607815, 607816, 611778, 611779, 611780, 627274 });

        // Soothing Kiss: ranks 1..5
        reconcile({ 6360, 7813, 11784, 11785, 27275 }, { 606360, 607813, 611784, 611785, 627275 });

        // Seduction & Lesser Invisibility (single ranks)
        reconcile({ 6358 }, { 606358 });
        reconcile({ 7870 }, { 607870 });
        break;

    case NPC_FELHUNTER:
        // Devour Magic + Shadow Bite
        reconcile({ 19505, 19731, 19734, 19736, 27276, 27277, 48011, 54049, 54050, 54051, 54052, 54053 }, { 619505, 619731, 619734, 619736, 627276, 627277 });

        // Tainted Blood / Fel Intelligence
        reconcile({ 20429, 20430, 20431, 20432, 27497, 54424, 57564, 57565, 57566, 57567 }, { 620429, 620430, 620431, 620432, 627497 });

        // Spell Lock: ranks 1..3
        reconcile({ 19244, 19647, 24259 }, { 619244, 619647 });

        // Paranoia (passive)
        reconcile({ 19481 }, { 619481 });
        break;

    case NPC_FELGUARD:
        // Intercept: ranks 1..3
        reconcile({ 30151, 30154, 30199, 30200 }, { 630154, 630199, 630200 });

        // Cleave: ranks 1..3
        reconcile({ 30213, 30214, 30222, 30224 }, { 630214, 630222, 630224 });

        // Anguish: ranks 1..3
        reconcile({ 33704, 33705, 33706 }, { 633704, 633705, 633706 });

        // Avoidance & Demonic Frenzy (passives)
        reconcile({ 32234 }, { 632234 });
        reconcile({ 32852 }, { 632852 });
        break;
    }
}
    ...
    bool OnPlayerCanCastItemUseSpell(Player* player, Item* item, SpellCastTargets const& /*targets*/, uint8 /*cast_count*/, uint32 /*glyphIndex*/) override
    {
        ...
        for (uint16 itemId : WARLOCK_GRIMOIRES)
        {
            if (item->GetEntry() == itemId)
            {
                ...
                auto checkGrimoire = [this, player, item](auto const& ids) -> int
                    {
                        ...
                        for (uint16 id : ids)
                        {
                            if (item->GetEntry() == id)
                            {
                                ...
                                {
                                    ChatHandler(player->GetSession()).PSendSysMessage("你的宠物已经学会了这个技能。");
                                    return -1;
                                }
                                else if (prevRank == 0)
                                ...
                                else
                                {
                                    ...
                                    {
                                        ChatHandler(player->GetSession()).PSendSysMessage("你的宠物需要先学习前一个等级。");
                                        return -1;
                                    }
                                }
                            }
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("你的恶魔需要靠近才能学习这个技能。");
                return false;
            }
        }
        return true;
            ...
            if (spell->GetSpellInfo()->Id == SPELL_TAME_BEAST)
            {
                ...
                for (uint8 i = 0; i < MAX_CREATURE_SPELL_DATA_SLOT; ++i)
                {
                    ...
                    if (!player->HasSpell(PetSpell + 600000))
                    {
                        ...
                        ChatHandler(player->GetSession()).PSendSysMessage("您已学会技能: {}（等级 {}）。", SpellName, SpellRank);
                    }
                }
        ...
        if (player->getClass() == CLASS_DRUID)
        {
            if (spell->GetSpellInfo()->SchoolMask == 8) // Nature
            {
                ...
                for (uint16 spellId : Druid_SM_8)
                {
                    ...
                    {
                        ChatHandler(player->GetSession()).PSendSysMessage("此技能在巫妖王之怒期间不可用。");
                        spell->cancel();
                        break;
                    }
                }
...
// Per-pet flag: a DelayedPetSpellCheck is already queued on this pet.
// Lives in the Pet's CustomData, so it dies with the Pet object and never leaks.
class PendingPetSpellCheck : public DataMap::Base
{
public:
    bool pending = false;
};
...
{
private:
    static constexpr char const* PENDING_CHECK_KEY = "ipp_PendingPetSpellCheck";
...
public:
    ...
    public:
        ...
        bool Execute(uint64 /*e_time*/, uint32 /*p_time*/) override
        {
            ...
            // The check runs now - allow the next pet load to queue a fresh one.
            pet->CustomData.GetDefault<PendingPetSpellCheck>(PENDING_CHECK_KEY)->pending = false;
